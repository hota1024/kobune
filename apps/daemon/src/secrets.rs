...
mod tests {
    ...
    async fn resolves_from_the_daemon_environment() {
        ...
        unsafe { std::env::set_var("KOBUNE_TEST_SECRET", "s3cret") };

        let resolved = resolve(&[(
            "API_KEY".to_string(),
            SecretRef::Env("KOBUNE_TEST_SECRET".into()),
        )])
        .await;
    ...
    async fn missing_environment_variable_is_reported_not_fatal() {
        let resolved = resolve(&[(
            "API_KEY".to_string(),
            SecretRef::Env("KOBUNE_TEST_DEFINITELY_UNSET".into()),
        )])
        .await;
    ...
    async fn other_keys_survive_one_failure() {
        ...
        unsafe { std::env::set_var("KOBUNE_TEST_OK", "fine") };

        let resolved = resolve(&[
            (
                "BAD".to_string(),
                SecretRef::Env("KOBUNE_TEST_MISSING".into()),
            ),
            ("GOOD".to_string(), SecretRef::Env("KOBUNE_TEST_OK".into())),
        ])
        .await;
    ...
    async fn missing_program_is_reported_clearly() {
        let err = run("kobune-definitely-not-a-program", &[])
            .await
            .unwrap_err();
    ...
    async fn identical_references_are_fetched_once() {
        unsafe { std::env::set_var("KOBUNE_TEST_SHARED", "shared") };

        let reference = SecretRef::Env("KOBUNE_TEST_SHARED".into());
        let resolved = resolve(&[
