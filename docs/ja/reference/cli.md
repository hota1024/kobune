...
| `starting` | コンテナは起動しているが、中のアプリがまだ応答していない |
| `ready` | 応答している。推測ではなく実際に確認します（[`health`](./minato-toml#起動完了の判定) を参照） |
| `failed` | 異常終了した。`reason` に理由が入ります |

**`ready` は「実際に到達できた」という意味です。** コンテナが起動している
ことと、中のアプリが応答できることは別だからです。そのため ready と報告する
前に必ず確認します。`health` を安価に評価できる場合はそれを、できない場合は
接続の可否を使います。ビルド中の dev server は `starting` になります。待つ
べきかどうかを判断できる状態はこちらです。
