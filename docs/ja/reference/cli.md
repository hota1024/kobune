...
送らず、ホイールは何も起こしません。アタッチ時にプログラムが端末に対して行った
設定を読み直してここで設定し直すため、turborepo のログペインは手元で直接
実行したときと同じようにポインタの下でスクロールします。デタッチすると元に
戻します。
...
launchd 経由になり——これが 80/443 番ポートを保持したままにする仕組みです——
`stop` はマシンから daemon を消すのではなく、ポートに何か届くまでジョブを
待機状態にします。
...
一覧を取得できなかったストレージ——インストール済みなのに応答しないランタイム
——は「無し」として扱わず報告します。削除できなかったボリュームも同様です。
