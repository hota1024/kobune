...
| 引数なしの `kobune` | 描画できる端末があれば全画面。[ダッシュボード](#ダッシュボード)を参照 |
...
あわせて `kobune.local.toml` と `.kobune/env.local` を `.gitignore` に追加し、
ファイルが無ければ作成します。どちらも 1 台のマシンに属するファイルで、
コミットされた時点で目的が失われるためです。git が既にその名前を対象にして
いる場合は追記しないため、再実行しても記述は増えません。`--force` が対象と
するのは `kobune.toml` だけです。
...
引数なしの `kobune` は全画面を開きます。左に workspace の一覧が並び、右には
カーソルのある workspace の詳細が出ます。一覧は 3 秒ごとに読み直します。
これがこの画面の存在理由です。scale-to-zero は指示がなくてもサービスを停止
しますし、別の端末から起動されることもあるため、一度出力したものは数秒で
古くなります。

```console
$ kobune                 # カレントディレクトリの workspace
$ kobune tui -w feat-1   # workspace を指定する
```

カーソルは、カレントディレクトリが属する workspace の上から始まります。
対象の決め方は他のコマンドと同じです。

| キー | 動作 |
| --- | --- |
| `↑` `↓`、`j` `k` | カーソルを移動する。ログペインではスクロールする |
| `tab`、`←` `→` | 画面に出ているペインを行き来する |
| `u` | カーソルのあるものを起動する。サービスのカーソルがあればそのサービス 1 つ、なければ workspace 全体 |
| `d` | 同じ範囲で停止する |
| `o` | URL をブラウザで開く |
| `l` | カーソルのあるもののログを下のペインで追う |
| `L` | そのペインを全画面にする。もう一度押すと戻る |
| `pg up`、`pg dn` | 1 画面分ずつ動かす |
| `g`、`G` | 先頭と末尾 |
| `c` | `kobune doctor` と同じ点検結果を出す |
| `e` | 環境変数を伏せ字で出す |
| `Q` | URL を撮影できるコードにして出す |
| `r` | 一覧をすぐ読み直す |
| `?` | キー一覧 |
| `q`、`ctrl-c` | 終了する |
| `esc` | 1 段階戻る。重ねた表示、全画面、ログペイン、ダッシュボードの順 |

起動と停止は同時に 1 つだけ走ります。その進捗は最下行に流れます。`kobune up`
が出力するものと同じ内容です。失敗した理由は、次に何かが起きるまでその行に
残ります。

### 重ねて表示されるもの

`c`、`e`、`Q`、`?` はダッシュボードの手前に枠を重ねます。いずれも印字側の
コマンドが既に持っていた view です。`kobune doctor`、`kobune env ls`、
`kobune url --qr` がそれぞれ出力するものが、そのまま枠の中に出ます。

枠が出ている間、矢印キーは後ろのカーソルではなく枠の中を動かします。入り
きらない場合は枠の端に `↑` または `↓` が出ます。開いたキーをもう一度押すと
閉じ、`esc` でも閉じます。それ以外のキーは、枠を閉じたうえで本来の動作を
します。スクロールできない表示より困るのは、閉じられない表示だからです。

**`e` の値は伏せ字で、表に戻すキーはありません。**
[シークレット](../guide/environment-variables#シークレット)を 1Password や
Keychain から解決した値だからです。ダッシュボードは、後ろに人が立てる画面
でもあります。`kobune env ls --reveal` は意図して打つコマンドであり、その
ままにします。

`Q` は daemon に何も尋ねません。URL は一覧が既に持っているからです。トンネル
が有効ならトンネル URL を、そうでなければその旨を出します。`.localhost` の
名前を解決できるのはこのマシンだけなので、撮影したスマートフォンからは何も
返ってこないためです。

### ログペイン

`l` を押すと、カーソルのあるものを対象に開きます。サービスのカーソルがあれば
そのサービス 1 つ、なければ workspace の全サービスです。同じ対象でもう一度
`l` を押すと閉じます。直近 200 行を表示し、そこから追尾します。

**対象は開いたときのものに固定されます。** そのあとカーソルを動かしても、
変わるのは上のペインの表示だけで、ログはそのままです。2 つの workspace を
見比べても流れが切れません。別の場所で `l` を押すと、そちらへ移ります。

上にスクロールすると追尾が止まり、`paused` と、その下に何行溜まったかを
表示します。`G` で最新行に戻り、追尾を再開します。読んでいる位置が新しい行に
押し流されることはありません。

ペインの幅を超える行は、切らずに折り返します。2 行目以降はサービス名の列の
下に字下げして揃えるため、折り返しても 1 行として読めます。ウィンドウの端で
失われる文字はありません。スタックトレースは、まさに幅を超える種類の行です。

色はそのまま出ます。開発サーバの出力はほとんどがエスケープシーケンスです。
Turborepo のプレフィックスや、緑色のステータスコードがそれにあたります。
これらは文字として出力せずに解釈するため、端末で見たときと同じ見え方に
なります。カーソル移動や画面消去など、端末が**動作として**扱うものは
捨てます。ペインにはカーソルも画面もないからです。

**読むだけです。** 入力はサービスに届きません。端末を貸し出すのは
`kobune logs -f` のほうで、この 2 つは別物です。
[サービスに入力する](#サービスに入力する)を参照してください。

ペインが保持するのは 2000 行です。それを超えると古い行から消えます。
全部を見たいときのために `kobune logs` があります。

**終了しても何も止まりません。** サービスは動き続けますし、完了していない
起動・停止もそのまま続きます。端末より環境が長生きすることが、daemon を
置いている理由です。

**端末が必要です。** パイプ、リダイレクト、`TERM=dumb`、`--json` のいずれかに
あたる場合、引数なしの `kobune` は従来どおりヘルプを出力します。`kobune tui` と
明示した場合は、何が問題かを出力します。端末が必要であること、あるいは `--json`
...
設定は 3 つのファイルからなり、順に読み込んで統合します。後のものが優先され、
`~/.kobune/config.toml`、プロジェクトの `kobune.toml`、その隣の
`kobune.local.toml` の順です。統合した結果は開けるファイルとしては存在しない
ため、それを確認するためのコマンドです。
...
**ファイルが無い層も、探した場所とともに表示します。** 「上書きが効かない」の
原因はたいてい「ファイルが別の場所にある」ことで、それを示せるのはパスだけ
です。3 つのうち 2 つは無いのが普通なので、`not found` は異常ではありません。

層の下には、複数の層が値を持っていたキーをドット区切りのパスで並べ、優先された
層と、それが上書きした層を添えます。`--all` を指定するとすべてのキーを表示し
ます。指定しない場合に絞り込むのは、ある程度の規模の設定では、競合している
数個が埋もれてしまうためです。

**設定が読み込めない状態でも動きます。** 統合した結果が設定として成立しない
場合、たとえばキーの綴りが違う、あるいは 2 つの層が矛盾する場合、他のコマンドは
そのメッセージだけを表示して停止します。このコマンドは読み込んだファイルと層ごと
の設定内容を表示し、その下にメッセージを添えます。まさにその状態で問われること
が多いためです。TOML として壊れているファイルがある場合は、このコマンドも失敗
します。エラーが該当ファイルを名指ししており、説明すべき統合結果もまだ無いから
です。

各ファイルの役割は [層](./kobune-toml#層) を参照してください。
...
| 変数 | 説明 |
| --- | --- |
| `KOBUNE_HOME` | 状態、ログ、ソケット、CA、`config.toml` の保存先。既定値 `~/.kobune` |
