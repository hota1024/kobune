...
全体を 1 つのウィンドウで見たい場合は
[`kobune studio`](../reference/cli#kobune-studio) です。ターミナルのものと同じ
ダッシュボードをブラウザに配信するもので、このアプリと違いリリースアーカイブに
同梱されています。
...
**daemon を起動することはありません。** daemon の管理は launchd の役割であり、
GUI が重ねて管理すると責務が分散します。接続できないと表示された場合は、CLI
から daemon を起動するか、LaunchDaemon を設定して常時稼働させてください。
