...
**daemon は再起動時に復元しません。** 復元しても誰も知らないホスト名を新しく
取得するだけで、共有済みのリンクが指しているのは消えた古い名前だからです。
そのため `kobune daemon restart` のあと、状態は `disabled` になります。再開は
明示的に実行します。
...
できます。quick は上記の理由から無効になります。`stopped` のまま残すと、
正しい状態について `doctor` が赤い印を出し続けることになるためです。
