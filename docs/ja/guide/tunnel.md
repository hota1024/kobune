...
場合はその旨を表示します。この状態は他のどこを見ても異常に見えない——トンネルは
起動し、`status` は `running` を返し、URL にはいつまでも何も届かない——ためです。
ゾーンを切り替えるには login をやり直してください。
...
ここまでの表示が `running` のままで、すべてのホスト名が別の場所へ流れます。URL を
信用する前に、ダッシュボードでそのレコードを確認してください。
