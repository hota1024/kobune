...
引数なしの `kobune` は全画面を開き、数秒ごとに読み直します。画面に出ているの
は Enter を押した時点の状態ではなく、今の状態です。`u` と `d` でカーソルのある
...
同じことは[ダッシュボード](../reference/cli#ダッシュボード)のペインでもできま
す。`l` でカーソルのあるものを追い、状態の表示はそのまま残ります。`L` で全画面
になります。保持するのは直近 2000 行なので、全部を見たいときやパイプに流したい
ときは `kobune logs` を使います。
