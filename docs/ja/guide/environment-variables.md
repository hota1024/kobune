...
解決できない値があっても `minato env ls` は一覧を表示します。値を書かれたまま
表示し、その理由を下に添えます。原因の値は値を眺めてしか見つけられないため、
一覧そのものが届かないと調べようがないからです。
...
サービス自体は問題なく起動します。その場合はその旨と、解決できる一覧である
`minato env ls --service <name>` を併せて表示します。
