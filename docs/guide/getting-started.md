...
`kobune doctor` has nothing to say.
...
`kobune init` writes a starter file and guesses the project name from the
...
- **`port`** is the port your app listens on *inside* the container. Kobune
...
Commit it. `kobune.toml` belongs in the repository — every worktree reads the
same one.
...
That last step — `waiting for web` — is Kobune waiting for your app to answer,
...
With a service named, `kobune url` prints one line and nothing else, so it
...
`kobune doctor` — it prints the command to fix it. This is the single most
...
$ kobune status               # what is running, and where
```
...
You never have to run `kobune up` again for a branch you are still using. This
