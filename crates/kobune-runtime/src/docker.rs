...
use context::{Dockerfile, Remarked, packing_failure, stream_context};
...
impl DockerRuntime {
    ...
    pub fn connect() -> Result<Self> {
        ...
        Ok(Self::new(docker, builds))
    }

    /// **Both clients or neither.** The longer leash on builds is the
    /// whole reason there are two, and a constructor that took one and
    /// filled the other in for itself would hand builds the two-minute
    /// bound back without saying so. Private, so [`Self::connect`] stays
    /// the only way in and the pair is always resolved together.
    fn new(docker: Docker, builds: Docker) -> Self {
        Self {
            docker,
            builds,
    ...
    /// **A body Kobune could not read is not a daemon it could not reach.**
    ///
    /// `bollard`'s models name the values Docker had when they were
    /// generated, and its enums carry no fallback variant — so a single
    /// container in a state this build has never heard of fails the whole
    /// response rather than that one container. A Podman socket behind
    /// `DOCKER_HOST` and a state a later Docker adds both land here.
    ///
    /// Nothing in this crate can parse it after the fact: `list_containers`
    /// hands back a decoded `Vec`, and the request machinery that could
    /// decode it leniently is `pub(crate)` to `bollard`. What is in reach
    /// is not misnaming it — reported as unreachable it sends the reader
    /// off to check a daemon that is up and answering, while the value
    /// nobody could parse is named in the message.
    fn unavailable(err: bollard::errors::Error) -> RuntimeError {
        use bollard::errors::Error as Bollard;

        match err {
            err @ (Bollard::JsonDataError { .. } | Bollard::JsonSerdeError { .. }) => {
                RuntimeError::caused_by("reading what Docker answered", &err)
            }
    ...
    async fn ensure_built(
        ...
        // **One remark per build, not per attempt.** A daemon without
        // BuildKit is found out only after the context has gone up once,
        // and the fallback below packs the very same worktree again. What
        // the remark is about is the context, so it outlives the attempt
        // that discovered it. See [`ChunkWriter::announce`].
        ...
        if !self.buildkit_ruled_out() {
            match self
                .run_build(build, &dockerfile, true, &label, events, &remarked)
                .await
            {
                Ok(()) => {
        ...
        // remembered above.
        self.run_build(build, &dockerfile, false, &label, events, &remarked)
            .await
    ...
    async fn run_build(
        ...
        let (chunks, packing) = stream_context(&build.context, dockerfile, label, events, remarked);
...
mod tests {
    ...
    /// **A state Docker knows and `bollard` does not takes the lot with
    /// it.**
    ///
    /// Pinned rather than fixed, because it cannot be fixed here: the
    /// generated enum has no `#[serde(other)]`, and one unknown value
    /// fails the whole `list_containers` body — every service in the
    /// workspace, not the one container in that state. If `bollard` ever
    /// grows a fallback this test starts failing, which is the day
    /// [`DockerRuntime::unavailable`] can lose half its comment.
    #[test]
    fn one_state_bollard_does_not_know_costs_the_whole_answer() {
        let body = r#"[{"Id":"a","State":"running"},{"Id":"b","State":"degraded"}]"#;

        let err = serde_json::from_str::<Vec<ContainerSummary>>(body)
            .expect_err("bollard now tolerates an unknown container state");

        assert!(
            err.to_string().contains("degraded"),
            "the error does not name the state nobody could read: {err}"
        );
    }

    /// **Read as what it is, not as Docker being down.**
    ///
    /// The daemon answered — it is the answer that could not be parsed. A
    /// reader told to check whether Docker is running would find it
    /// running and be no further on.
    #[test]
    fn an_answer_that_could_not_be_read_is_not_a_daemon_that_could_not_be_reached() {
        let err = DockerRuntime::unavailable(bollard::errors::Error::JsonDataError {
            message: "unknown variant `degraded`".to_string(),
            column: 58,
        });

        assert!(
            !matches!(err, RuntimeError::Unavailable { .. }),
            "a body that would not parse was reported as an unreachable daemon: {err}"
        );
        assert!(
            err.to_string().contains("degraded"),
            "the report does not name what could not be read: {err}"
        );

        // A daemon that really is not there still says so.
        let down = DockerRuntime::unavailable(bollard::errors::Error::RequestTimeoutError);
        assert!(
            matches!(down, RuntimeError::Unavailable { .. }),
            "a daemon that did not answer stopped being reported as unreachable: {down}"
        );
    }
